...
void Internal::circuit_transred() {
    ...
    if (i == end) {
        ...
        for (i = circuit_gates.begin (); i != end; i++) {
            Circuit_Gate *c = *i;
            if (c->transred)
                c->transred = false;
            }
        i = circuit_gates.begin ();
    }
    ...
    while (!unsat && i != end && !terminated_asynchronously () &&
        ...
        if (circuit_watches(src).size() < circuit_watches(dst).size()) {
            int tmp = dst;
            dst = src;
            src = tmp;
        }
