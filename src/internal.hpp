...
#include <array>
#include <queue>
#include "../src_circuit/circuit_gate.hpp"
#include "../src_circuit/circuit_aig.hpp"
#include "../src_circuit/circuit_parser.hpp"
#include "../src_circuit/circuit_watch.hpp"
#include "../src_circuit/circuit_occs.hpp"
#include "../src_circuit/circuit_bins.hpp"

#include "../src_easysat/EasySAT_heap.hpp"
...
/// (taomengxia): member for circuit solver
public:
    vector<Circuit_Watches> circuit_wtab;                 // watch table
    vector<vector<int>> circuit_dwtab;                    // direct watch table


    Circuit_Gate* circuit_conflict_gate = nullptr;
    std::array<int, 2> circuit_conflict_direct{0, 0};

    Circuit_Gate *circuit_probe_reason = nullptr;                 // set during probing
    int circuit_probe_reason_direct = 0;

    std::vector<Circuit_Gate*> circuit_gates;

    vector<Circuit_Occs> circuit_otab;            // table of occurrences for all literals
    vector<Circuit_Bins> circuit_big;             // binary implication graph

    bool circuit_occurring () const { return !circuit_otab.empty (); }

    Circuit_Occs &circuit_occs(int lit) { return circuit_otab[vlit (lit)]; }
    Circuit_Bins &circuit_bins(int lit) { return circuit_big[vlit (lit)]; }

/// (taomengxia): API for circuit solver begin
public:
    signed char circuit_val(int lit) const { return val(lit); }
    void circuit_set_val(int lit, signed char val) { set_val(lit, val); }

    int circuit_assign_level_get(int lit)             { return var(lit).level; }

    // Marking and unmarking of all literals in a gate.
    //
    void circuit_mark(Circuit_Gate*);
    void circuit_unmark(Circuit_Gate*);

    /// API related to circuit_wtab: circuit_wtab with fixed-size (2 * max_var)
    bool circuit_watching() const { return !circuit_wtab.empty (); }
    Circuit_Watches& circuit_watches(int lit) { return circuit_wtab[vlit(lit)]; }

    inline void circuit_watch_literal(int lit, int blit, Circuit_Gate *g) {  // interal.hpp watch_literal()
        Circuit_Watches &ws = circuit_watches (lit);
        ws.push_back(Circuit_Watch(blit, g));
    }

    inline void circuit_watch_gate(Circuit_Gate *g) {    // internal.hpp watch_clause()
        const int l0 = g->literals[0];
        const int l1 = g->literals[1];
        circuit_watch_literal(l0, l1, g);
        circuit_watch_literal(l1, l0, g);
    }

    inline void circuit_watch_gate_direct(Circuit_Gate *g) {
        // 构造一跳向左直接蕴含图
        // AND节点向左蕴含规则：输出为非监视值，可以蕴含出两个输入都为监视值
        auto input1 = g->literals[0];
        auto input2 = g->literals[1];
        auto output = g->literals[2];

        auto& left_dimp = circuit_direct_watches(-output);
        left_dimp.push_back(input1);
        left_dimp.push_back(input2);

        // 构造一跳向右直接蕴含图
        // AND节点向右蕴含规则：输入有一个为非监视值，可以蕴含出输出为监视值
        auto& direct_input1 = circuit_direct_watches(-input1);
        direct_input1.push_back(output);

        auto &direct_input2 = circuit_direct_watches(-input2);
        direct_input2.push_back(output);
    }

    vector<int>& circuit_direct_watches(int lit)  { return circuit_dwtab[vlit(lit)]; }

    void circuit_conflict_clear() {
        circuit_conflict_gate = nullptr;
        circuit_conflict_direct.fill(0);
    }

    /// API: implement in circuit_internal.cpp
    void circuit_init(const Circuit_Graph *grap_ptr);
    int circuit_solve();
    int circuit_cdcl_loop_with_inprocessing();

    /// analyze API: implement in circuit_analyze.cpp
    void circuit_learn_empty_clause();
    void circuit_learn_unit_clause(int lit);
    void circuit_bump_variables();
    int circuit_recompute_glue(Circuit_Gate* gate);
    void circuit_bump_gate(Circuit_Gate *g);
    bool circuit_bump_also_reason_literal(int lit);
    void circuit_bump_also_reason_literals(int lit, int limit);
    void circuit_bump_also_all_reason_literals();
    void circuit_analyze_literal(int lit, int &open, int &resolvent_size, int &antecedent_size);
    void circuit_analyze_reason(int lit, Circuit_Gate *reason, const std::array<int, 2> &reason_direct_array,
                                int &open, int &resolvent_size, int &antecedent_siz);
    Circuit_Gate* circuit_new_driving_gate(int lbd, int& jump);
    int circuit_find_conflict_level(int &forced);
    int circuit_determine_actual_backtrack_level(int jump);
    void circuit_analyze();
    void circuit_iterate();

    void circuit_eagerly_subsume_recently_learned_clauses(Circuit_Gate *g);

    /// backtrack API: implement in backtrack.cpp/circuit_backtrack.cpp
    void circuit_unassign(int lit);
    void circuit_backtrack(int target_level = 0);

    /// decide API: implement in circuit_decide.cpp
    bool circuit_satisfied();
    int circuit_decide_phase(int idx, bool target);
    int circuit_likely_phase(int idx);
    int circuit_decide();

    /// gates_info API: implement in circuit_gate.cpp
    Circuit_Gate* circuit_new_gate(bool red, int lbd);
    void circuit_promote_gate(Circuit_Gate *g, int new_glue);
    size_t circuit_shrink_gate(Circuit_Gate *g, int new_size);
    void circuit_deallocate_gate(Circuit_Gate *g);
    void circuit_delete_gate(Circuit_Gate *g);
    void circuit_mark_garbage(Circuit_Gate *g);
    void circuit_assign_original_unit(int lit);
    Circuit_Gate *circuit_new_learned_redundant_gate(int glue);
    Circuit_Gate *circuit_new_hyper_binary_resolved_clause(bool red, int glue);

    /// propagate API: implement in circuit_propagate.cpp
    int circuit_assignment_level(int lit, Circuit_Gate *reason);
    void circuit_search_assign(int lit, Circuit_Gate *reason, int reason_direct);
    void circuit_search_assume_decision(int lit);
    void circuit_search_assign_driving(int lit, Circuit_Gate* reason, int reason_direct);
    void circuit_assign_unit(int lit);
    bool circuit_propagate();
    void circuit_propagate_direct(int lit, int direct);

    /// reduce API: implement in circuit_reduce.cpp
    bool circuit_reducing();
    void circuit_mark_useless_redundant_gates_as_garbage();
    bool circuit_propagate_out_of_order_units();
    void circuit_reduce();

    /// collect API: implement in circuit_collect.cpp
    int gate_contains_fixed_literal(Circuit_Gate *g);
    void circuit_remove_watched_literals(Circuit_Gate *g);
    void circuit_mark_satisfied_gates_as_garbage();
    void circuit_flush_direct_watches(int lit);
    void circuit_protect_reasons();
    void circuit_unprotect_reasons();
    void circuit_flush_watches(int lit, Circuit_Watches &saved);
    void circuit_flush_all_occs_and_watches();
    void circuit_update_reason_references();
    void circuit_delete_garbage_gates();
    void circuit_copy_gate(Circuit_Gate *g);
    void circuit_copy_non_garbage_gates();
    bool circuit_arenaing();
    void circuit_garbage_collection();

    /// rephase API: implement in circuit_rephase.cpp
    bool circuit_rephasing();
    char circuit_rephase_original();
    char circuit_rephase_inverted();
    char circuit_rephase_flipping();
    char circuit_rephase_random();
    char circuit_rephase_best();
    char circuit_rephase_walk();
    void circuit_rephase();

    /// restart API: implement in circuit_restart.cpp
    bool circuit_restarting();
    void circuit_restart();

    /// minimize API: implement in circuit_minimize.cpp
    bool circuit_minimize_literal(int gate_id, int depth = 0);
    void circuit_minimize_clause();
    void circuit_minimize_sort_clause();
    void circuit_clear_minimized_literals();

    /// shrink API: implement in circuit_shrink.cpp
    void circuit_shrink_and_minimize_clause();
    void circuit_reset_shrinkable();
    void circuit_mark_shrinkable_as_removable(int blevel, std::vector<int>::size_type minimized_start);
    int circuit_shrink_literal(int lit, int blevel, unsigned max_trail);
    unsigned circuit_shrunken_block_uip(int uip, int blevel,
                                        std::vector<int>::reverse_iterator &,
                                        std::vector<int>::reverse_iterator &,
                                        std::vector<int>::size_type minimized_start,
                                        const int uip0);
    void circuit_shrunken_block_no_uip(const std::vector<int>::reverse_iterator &,
                                       const std::vector<int>::reverse_iterator &,
                                       unsigned &block_minimized, const int uip0);
    void circuit_push_literals_of_block(const std::vector<int>::reverse_iterator &,
                                        const std::vector<int>::reverse_iterator &,
                                        int blevel, unsigned max_trail);
    unsigned circuit_shrink_next(int, unsigned &, unsigned &);
    std::vector<int>::reverse_iterator circuit_minimize_and_shrink_block (std::vector<int>::reverse_iterator &,
                                                                          unsigned int &, unsigned int &,
                                                                          const int);
    unsigned circuit_shrink_block (std::vector<int>::reverse_iterator &,
                                   std::vector<int>::reverse_iterator &,
                                   int blevel, unsigned &open, unsigned &block_minimized,
                                   const int uip, unsigned max_trail);
    unsigned circuit_shrink_along_reason (int, int, bool, bool &, unsigned);


    /// probe API: implement in circuit_probe.cpp
    bool circuit_probing ();
    void circuit_failed_literal (int lit);
    void circuit_probe_lrat_for_units (int lit);
    void circuit_probe_assign_unit (int lit);
    void circuit_probe_assign_decision (int lit);
    void circuit_probe_assign (int lit, int parent);
    void circuit_mark_duplicated_binary_clauses_as_garbage ();
    int circuit_get_parent_reason_literal (int lit);
    void circuit_set_parent_reason_literal (int lit, int reason);
    void circuit_clean_probehbr_lrat ();
    void circuit_init_probehbr_lrat ();
    int circuit_probe_dominator (int a, int b);
    int circuit_hyper_binary_resolve (Circuit_Gate *reason);
    void circuit_probe_propagate2 ();
    void circuit_probe_propagate_direct(int lit, int direct);
    void circuit_probe_propagate_watch_list (int lit);
    bool circuit_probe_propagate ();
    bool circuit_is_binary_gate (Circuit_Gate *g, int &, int &);
    void circuit_calculate_noccs_in_binary();
    void circuit_generate_probes ();
    void circuit_flush_probes ();
    int circuit_next_probe ();
    bool circuit_probe_round ();
    void circuit_probe (bool update_limits = true);

    /// subsume API: implement in circuit_subsume.cpp
    bool circuit_subsuming ();
    void circuit_strengthen_clause(Circuit_Gate *c, int lit);
    void circuit_subsume_clause(Circuit_Gate *subsuming, Circuit_Gate *subsumed);
    int circuit_subsume_check (Circuit_Gate *subsuming, Circuit_Gate *subsumed);
    int circuit_try_to_subsume_clause (Circuit_Gate *, vector<Circuit_Gate *> &shrunken);
    bool circuit_subsume_round ();
    void circuit_subsume (bool update_limits = true);

    /// Bounded variable elimination API: implement in circuit_elim.cpp
    bool circuit_eliminating ();
    void circuit_elim (bool update_limits = true);

    /// Operators on circuit_watches in circuit_watch.cpp
    void circuit_init_watches ();
    void circuit_connect_watches (bool irredundant_only = false);
    void circuit_sort_watches ();
    void circuit_clear_watches ();
    void circuit_reset_watches ();

    /// decompose API: implement in circuit_decompose.cpp
    void circuit_decompose ();

    /// ternary API: implement in circuit_ternart.cpp
    bool circuit_ternary();


    /// Transitive reduction of binary implication graph in 'circuit_transred.cpp'
    void circuit_transred();

    /// @note:  internal.hpp:  likely_to_be_kept_clause()
    bool circuit_likely_to_be_kept_clause(Circuit_Gate *g) {
        if (!g->redundant)
            return true;
        if (g->keep)
            return true;
        if (g->glue > lim.keptglue)
            return false;
        if (g->size > lim.keptsize)
            return false;
        return true;
    }

    void circuit_mark_added (int lit, int size, bool redundant);
    void circuit_mark_added (Circuit_Gate *);

    void circuit_init_occs();
    void circuit_init_bins();
    void circuit_reset_occs();
    void circuit_reset_bins();

/// (taomengxia): API for easysat solver begin
    EasySAT_Solver* easysat_solver;
    const char* easysat_parse(char *filename);
    int easysat_solve();
/// (taomengxia): API for easysat solver end
};
